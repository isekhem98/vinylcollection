...
def search_releases(query: str, token: str = "", page: int = 1, per_page: int = 10) -> list[dict]:
    ...
    out = []
    for r in results:
        images = r.get("cover_image", "") or r.get("thumb", "")
        out.append({
            ...
            "cover_image_url": r.get("thumb", "") or r.get("cover_image", ""),
...
def _parse_release(data: dict, release_id: str) -> dict:
    ...
    # Prefer uri150 (thumbnail) then uri
    images = data.get("images", [])
    cover = ""
    for img in images:
        if img.get("type") == "primary":
            cover = img.get("uri150") or img.get("uri", "")
            break
    if not cover and images:
        cover = images[0].get("uri150") or images[0].get("uri", "")
